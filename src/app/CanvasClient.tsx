...
export default function CanvasClient() {
  ...
  /** Where a picture lands when nothing said where — the middle of the view. */
  ...
  const placeImage = useCallback(
    async (file: File, opts: { nodeId?: string | null; world?: { x: number; y: number } } = {}) => {
      ...
      const at = opts.world ?? viewCentre();
      const node = withImage(
        {
          id: makeId(),
          type: "text",
          text: "",
          x: gridSnap(at.x - width / 2),
          y: gridSnap(at.y - height / 2),
          width,
          height,
        } as TextNode,
        key,
    ...
    [applyDoc, gridSnap, showToast, viewCentre],
  );
  ...
  // Pasting a picture is the desk-side way in, and it follows the same rule as
  // the button: onto the selected card, or onto the canvas as a new one.
  useEffect(() => {
    const onPaste = (e: ClipboardEvent) => {
      // The JSON sheet is a text surface; a paste there is a map, not a photo.
      if (pasteOpen || jumpOpen) return;
      ...
      void placeImage(file, { nodeId: ids.length === 1 ? ids[0] : null });
    };
    window.addEventListener("paste", onPaste);
    return () => window.removeEventListener("paste", onPaste);
  }, [jumpOpen, pasteOpen, placeImage]);
